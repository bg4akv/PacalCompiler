...
public class Dictionary {
	...
	private final Map<String, Tuple> dictionary = new HashMap<String, Tuple>();
	...
	protected void addToken(String name, Tuple tokenMeta)
	{
		dictionary.put(name, tokenMeta);
	}

	// if we can find it in the dictionary, we change the token type
	Tuple lookup(String name)
	{
		TokenValue tokenValue = TokenValue.UNRESERVED;
		TokenType tokenType = TokenType.IDENTIFIER;
		int precedence = -1;

		Tuple tuple = dictionary.get(name);
		if (tuple != null) {
